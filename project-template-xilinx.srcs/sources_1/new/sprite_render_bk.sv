...
(
    ...
    // dst, SRAM
    input [19:0] vm_start,      // 该显存开始的地址,和SRAM地址宽度一样
    input [11:0] hpos,          // 物体在显存中渲染的坐标
    input [11:0] vpos,
    // src, 从SDRAM中读取，存储到reg中
    input [7:0] line_buffer [63:0],
    ...
    localparam [3:0] IDLE=0, READ1=1, READ2=2, WRITE1=3, WRITE2=4, WRITE3=5, PRE_READ=6, PRE_WRITE=7,DONE=8,
            LOAD_LINE=9, RENDER=10, RENDER_DONE=11;
    reg [7:0] img_line_buffer[63:0];        //用于存储每个图片的行缓冲，32像素 = 64B = 8次SDRAM读取
