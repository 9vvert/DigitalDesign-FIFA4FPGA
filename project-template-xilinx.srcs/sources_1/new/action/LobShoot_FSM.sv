...
module LobShoot_FSM
// 相比于传球，射门没有转向动作
import type_declare::PlayerInfo, type_declare::FreeInit;
#(parameter SPEED_MIN=4, SPEED_MAX = 8, CHARG_T = 400)   //实际倍率 * 3
(
    input LobShoot_FSM_game_clk,
    input rst,
    ...
    output reg [2:0] shoot_level,       //蓄力等级，用于渲染动画
...
import LineLib::*;
    ...
    reg [3:0] LobShoot_FSM_stat;
    reg [9:0] accumulate_counter;       // 蓄力计数器
    reg [3:0] wait_counter;
    reg done_delay_counter;
    always@(posedge LobShoot_FSM_game_clk) begin
        if(rst) begin
            LobShoot_FSM_stat <= IDLE;
            shoot_done <= 0;
            ...
            if(LobShoot_FSM_stat == IDLE)begin
                shoot_done <= 0;
                shoot_signal <= 0;
                if(shoot_enable)begin
                    LobShoot_FSM_stat <= ACCUMULATE;
                    accumulate_counter <= 0;
                    shoot_level <= 1;
                end else begin
                    shoot_level <= 0;
                end 
            end else if(LobShoot_FSM_stat == ACCUMULATE)begin
                if(shoot_enable && hold)begin           // 蓄力
                    if(accumulate_counter >= CHARG_T)begin
                        accumulate_counter <= 0;
                    ...
                    free_init.init_vertical_signal <= 0;
                    free_init.init_vertical_speed <= 3;     // [TODO]数值速度，数值待测试
                    wait_counter <= 0;
                    LobShoot_FSM_stat <= WAIT;
                end else begin  // hold突然变成0，在正常的自动机过程中不会发生，除非有人抢断
                    LobShoot_FSM_stat <= DONE;   //异常终止
                end
            end else if(LobShoot_FSM_stat == WAIT)begin
                if(~hold)begin       // 不再持球，可能是成功射出，也可能被拦截。但是不关心结果，最后都会结束
                    shoot_signal <= 0;
                    LobShoot_FSM_stat <= DONE;
                end else begin
