...
/* Updates to the MIT version:
 * 1. Add initialization sequence(CMD8) to support SDHC cards
 * 2. Use a more natural(ready-valid with valid tied to 1) write interface
 */
...
module sd_controller(
    ...
    input wr,   // Write-enable. When [ready] is HIGH, asserting [wr] will
                ...
                // User should prepare data on [din] before [byte_sent]
                // transitions to HIGH. When [byte_sent] is transitioned to
                // HIGH, the user should advance to the next byte in the next
                // cycle until 512 bytes are sent.
    input [7:0] din, // Data input for WRITE operation.
    output reg byte_sent, // The byte on [din] is sent.
...
`endif
            ...
            case(state)
                ...
                WRITE_BLOCK_BYTE: begin
                    if (sclk_sig == 1) begin
                        if (bit_counter == 0) begin
                            if ((byte_counter != 2) && (byte_counter != 1) && byte_counter != WRITE_BLOCK_BYTE) begin
                                byte_sent <= 1;
                            end
                            state <= WRITE_BLOCK_DATA;
                        end
